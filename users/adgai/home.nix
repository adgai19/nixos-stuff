...
let 
  tmuxPlugin = with pkgs; [tmuxPlugins.cpu
      ...
      tmuxPlugins.yank];
  bumblebee-status = ps: ps.callPackage ./customPkgs/python/bumblebee-status;
  i3-workspace-names-daemon = ps: ps.callPackage ./customPkgs/python/i3-workspace-names-daemon;
  pythonPackages = python-packages: with python-packages; [pip (ps: with ps;[ bumblebee-status i3-workspace-names-daemon])];
  pythonWithPkgs = pkgs.python3.withPackages pythonPackages;
      in
{
...
# Let Home Manager install and manage itself.
  ...
  home.packages = with pkgs; [bottom tree ripgrep pavucontrol pythonWithPkgs cachix];
